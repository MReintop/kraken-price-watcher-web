...
- **The feed's state is said once, per screen; a row says only what is true of its own symbol.** The socket's status is global, so eight cards repeating it adds nothing — the markets list carries one banner and the rows carry per-symbol text. What every row does show is a muted price the moment `isPriceCurrent` goes false, because a dead feed's last price otherwise looks exactly like a live one. `connecting` is deliberately current: that number is the server's own seed, not a stale tick.
