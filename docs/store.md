...
store/krakenSocket.ts  one WebSocket for every tracked market; buffers + coalesces ticks
...
app/(markets)/layout.tsx (server component)
  └─ await getCoins()                    ← server fetch, once per request
      ...
      └─ <StoreProvider initialCoins>    ← 'use client' boundary
          ...
          ├─ startKrakenTicker(markets, store.dispatch)   ← TRACKED_MARKETS entries
...
- **One socket, not one per row.** `startKrakenTicker` subscribes to every market on a single connection.
...
- **`TRACKED_MARKETS` is the authority on every name a market has.** Coin identity (`id`), the store's key (`symbol`), the Kraken REST pair (`restPair`) and the Kraken WebSocket pair (`websocketPair`) are stated there, together, once. None of them is derived from a response: CoinGecko's `symbol` is display metadata, and building a subscription out of it couples one upstream's naming to another's protocol. It is also wrong — Kraken calls dogecoin `XDG/USD`, which CoinGecko has never called it, so the derived subscription asked for a market that does not exist and that row never ticked. `getCoins()` overwrites CoinGecko's symbol with the registry's for the same reason.
- **Symbols are UPPERCASE in the store, and come from the registry.** `seedPricesFromCoins` keys by `coin.symbol` (already the registry's, already uppercase); the socket resolves a tick's pair back through `TRACKED_MARKETS` rather than splitting the pair on `/`. Every component must call `selectPrice(symbol.toUpperCase())`. A casing or naming mismatch renders _nothing_ — no error, no warning.
- **An empty market list opens no socket.** An empty subscribe is never answered, so it would sit out the handshake deadline, close, and reconnect on the backoff forever. `startKrakenTicker` returns a no-op cleanup instead.
